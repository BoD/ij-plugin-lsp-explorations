...
public class GraphQLParser implements PsiParser, LightPsiParser {
    ...
    public static boolean stringLiteral(PsiBuilder builder, int level) {
        ...
        if (!result) result = stringLiteral_1(builder, level + 1);
        exit_section_(builder, level, marker, result, false, null);
        return result;
    }

    // blockString {}
    private static boolean stringLiteral_1(PsiBuilder builder, int level) {
        if (!recursion_guard_(builder, level, "stringLiteral_1")) return false;
        boolean result;
        Marker marker = enter_section_(builder);
        result = blockString(builder, level + 1);
        result = result && stringLiteral_1_1(builder, level + 1);
        exit_section_(builder, marker, null, result);
        return result;
    }

    // {}
    private static boolean stringLiteral_1_1(PsiBuilder builder, int level) {
        return true;
    }
